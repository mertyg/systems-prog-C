...
#define BUF_SIZE 128
#define USERNAME_MESSAGE "Please Enter a Username\r\n"
 ...
 */
int read_from(int client_index, struct sockname *usernames, fd_set *fs, int top) {
    int fd = usernames[client_index].sock_fd;
    char msg[BUF_SIZE + 1];
    int num_read = read(fd, &msg, BUF_SIZE);
    msg[num_read] = '\0';
    char buf[BUF_SIZE+num_read+1];
...
void read_username(int client_fd, struct sockname *usernames){
    int client_index = 0;
    while (usernames[client_index].sock_fd !=client_fd ) {
        client_index++;
    }
    ...
    //printf("Read username: %s",usernames[client_index].username);
    }
}
...
int main(void) {
    ...
    while (1) {
        ...
        if (FD_ISSET(sock_fd, &listen_fds)) {
            ...
            w = dprintf(client_fd,USERNAME_MESSAGE);
            if(w<strlen(USERNAME_MESSAGE)){
                perror("write");
                exit(1);
            }
            read_username(client_fd,usernames);
        }
